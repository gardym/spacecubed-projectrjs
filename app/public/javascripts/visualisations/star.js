  ...
  return function(event) {
    ...
    this.twinkle = function(star, scaleX, scaleY) {
      setInterval(function() {
        ...
        star.css('backgroundColor', randomColour());
        star.css('-webkit-transform',
          'translate3d(0, 0, 0) ' +
          'scaleX(' + (resize * scaleX) + ') ' +
          'scaleY(' + (resize * scaleY) + ')');
        star.css('borderRadius', star.width() * 0.8);
        star.css('opacity', Math.random() * 0.5);
        star.css('-webkit-transition',
          '-webkit-transform 500ms ease-in-out, opacity 500ms ease-in-out, background-color 500ms ease-in-out');
